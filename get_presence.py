...
def getAccessToken(clientId, refreshToken, redirectUri):
    ...
    payload = {'client_id': clientId, 'refresh_token': refreshToken,
               'grant_type': grant_type, 'scope': scope, 'redirect_uri': redirectUri}

    getAccessToken = requests.post(
        'https://login.microsoftonline.com/organizations/oauth2/v2.0/token', data=payload)
...
def getPresence(accessToken):
    ...
    getPresence = requests.get(
        'https://graph.microsoft.com/v1.0/me/presence', headers=presenceRequestHeaders)
...
mqttClient.username_pw_set(
    settingsJson['mqtt_username'], settingsJson['mqtt_password'])
...
while True:
        ...
        try:
            ...
            if tokenJson['expire_timestamp'] <= time.time():
                accessToken = getAccessToken(
                    settingsJson['client_id'], settingsJson['refresh_token'], settingsJson['redirect_uri'])
            else:
    ...
    time.sleep(settingsJson['check_every'])
